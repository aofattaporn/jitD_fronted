...
class CommentRepository {
  ...
  Future<String> creatingComment(String? content, String? commentId,
      String? postId, int? like, String? userId, String? date) async {
    String date = DateTime.now().toString();
    String? id = FirebaseAuth.instance.currentUser?.uid;
    String? token = await FirebaseAuth.instance.currentUser?.getIdToken();
    final response = await http.post(
        Uri.parse("${globalUrl}v1/comments/posts/$id"),
        headers: {
        ...
        body: commentModelToJson(
            CommentModel(content, commentId, postId, like, userId, date)));
